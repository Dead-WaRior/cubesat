...
function GroundTrack() {
   ...
   const pathPoints = useMemo(() => {
      if (!satPath || satPath.length < 2) return null;
      return satPath.map(p => {
         const r = Math.sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
         const z_over_r = Math.max(-1.0, Math.min(1.0, p.z / r));
         const lat = Math.asin(z_over_r) * (180 / Math.PI);
      ...
      <div className="glass-card rounded-xl overflow-hidden shadow-lg flex flex-col h-full">
         ...
         <div className="flex-1 relative bg-slate-950 p-2 min-h-[220px]">
            ...
            <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-full opacity-80">
               ...
               {pathPoints && (
                  <polyline points={pathPoints} fill="none" stroke="#22c55e" strokeWidth="1" strokeDasharray="3 3" opacity="0.6" />
               )}
